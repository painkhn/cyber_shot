    ...
    <div class="max-w-4xl w-full h-auto mx-auto my-0 mb-[120px]">
        ...
        <ul class="grid grid-cols-3 gap-14 gap-y-24">
            @foreach($categories as $category)
                <li>
                    <a href="{{ route('category.index', ['category' => $category->link]) }}" class="text-2xl w-full h-20 border-b border-black flex items-center justify-center transition-all hover:bg-transparent/5">
                        {{ $category->name }}
                    </a>
                </li>
            @endforeach
